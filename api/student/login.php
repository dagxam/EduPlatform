...
$stmt = $pdo->prepare(
    'SELECT u.id, u.first_name, u.last_name, u.is_active,
            ...
            ca.registration_open, cs.pin_hash, cs.activated_at
...
if ($student['activated_at'] === null) {
    if (!(int)$student['registration_open']) {
        json_response(['ok' => false, 'error' => 'Первичная регистрация для этого класса закрыта. Обратитесь к учителю.'], 403);
    }
...
json_response([
    'ok' => true,
    'first_login' => $student['activated_at'] === null,
    'user' => current_user(),
]);
