...
if ((int)$admin['is_platform_admin'] === 1) {
    json_response(['ok' => false, 'error' => 'Главный администратор UVORIA не назначается учителем отдельной школы.'], 409);
}
...
audit_event($enabled ? 'school_admin_granted_teacher' : 'school_admin_teacher_removed', 'user', $adminId, [], $schoolId, (int)$user['id']);

json_response(['ok' => true, 'can_teach' => $enabled]);
